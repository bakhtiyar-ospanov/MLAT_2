...
namespace Modules.Assets.Assistant
{
    public class AssistantAsset : Asset
    {
        ...
        public override void Init()
        {
            ...
            // Select random assistant mesh
            if (transform.childCount > 0)
            {
                foreach (Transform child in transform)
                    child.gameObject.SetActive(false);
                
                model = transform.GetChild(Random.Range(0, transform.childCount)).gameObject;
                model.SetActive(true);
                TextToSpeech.Instance.SetGenderVoice(TextToSpeech.Character.Assistant, model.name.EndsWith("-M") ? 1 : 0);
            }
            else
            {
                model = gameObject;
            ...
            _audioSource = model.AddComponent<AudioSource>();
            _smrs = transform.GetComponentsInChildren<SkinnedMeshRenderer>();
        ...
        {
            if(GetSMRandBlendIndex("Lip_Open").blendIndex == -1 ||
               GetSMRandBlendIndex("Tongue_Raise").blendIndex == -1 ||
               GetSMRandBlendIndex("Tongue_Out").blendIndex == -1 ||
               GetSMRandBlendIndex("Mouth_Bottom_Lip_Bite").blendIndex == -1 ||
               GetSMRandBlendIndex("Open").blendIndex == -1 ||
               GetSMRandBlendIndex("Tight").blendIndex == -1 ||
               GetSMRandBlendIndex("Mouth_Bottom_Lip_Under").blendIndex == -1 ||
               GetBone("JawRoot") == null)
                return;
            
            // Configure Salsa
            var salsa = model.AddComponent<Salsa>();
            salsa.audioSrc = _audioSource;
            salsa.queueProcessor = model.AddComponent<QueueProcessor>();
            salsa.useTimingsOverride = false;
            salsa.originalUpdateDelay = 0.08509f;
            salsa.visemes.Clear();
            
            // setup viseme 0 -- etc
            salsa.visemes.Add(new LipsyncExpression("etc", new InspectorControllerHelperData(), 0f));
            var etcLE = salsa.visemes[0].expData;

            var smrParams = GetSMRandBlendIndex("Lip_Open");
            ...
            // setup viseme 1 -- L
            salsa.visemes.Add(new LipsyncExpression("L", new InspectorControllerHelperData(), 0f));
            var LLE = salsa.visemes[1].expData;
            ...
            smrParams = GetSMRandBlendIndex("Tongue_Raise");
            ...
            smrParams = GetSMRandBlendIndex("Tongue_Out");
            LLE.controllerVars[2].smr = smrParams.smr;
            LLE.controllerVars[2].blendIndex = smrParams.blendIndex;
            
            // setup viseme 2 -- FV
            salsa.visemes.Add(new LipsyncExpression("FV", new InspectorControllerHelperData(), 0f));
            var FVLE = salsa.visemes[2].expData;
            smrParams = GetSMRandBlendIndex("Mouth_Bottom_Lip_Under");
            ...
            smrParams = GetSMRandBlendIndex("Mouth_Bottom_Lip_Bite");
            FVLE.controllerVars[1].smr = smrParams.smr;
            FVLE.controllerVars[1].blendIndex = smrParams.blendIndex;

            // setup viseme 3 -- O
            salsa.visemes.Add(new LipsyncExpression("O", new InspectorControllerHelperData(), 0f));
            var OLE = salsa.visemes[3].expData;
            ...
            smrParams = GetSMRandBlendIndex("Tight");
            ...
            // // setup viseme 4 -- AI
            salsa.visemes.Add(new LipsyncExpression("AI", new InspectorControllerHelperData(), 0f));
            var AILE = salsa.visemes[4].expData;
            ...
            smrParams = GetSMRandBlendIndex("Open");
            AILE.controllerVars[1].smr = smrParams.smr;
            AILE.controllerVars[1].blendIndex = smrParams.blendIndex;

            salsa.visemes[0].trigger = 0.013f;
            salsa.visemes[1].trigger = 0.116f;
            salsa.visemes[2].trigger = 0.291f;
            salsa.visemes[3].trigger = 0.397f;
            salsa.visemes[4].trigger = 0.513f;
            salsa.UpdateExpressionControllers();
        }

        private void SetupEyes()
        {
            if(GetSMRandBlendIndex("Eye_Blink").blendIndex == -1 ||
               GetBone("Head") == null ||
               GetBone("L_Eye") == null ||
               GetBone("R_Eye") == null)
                return;
            ...
            // System Properties
            _eyes = model.AddComponent<Eyes>();
            _eyes.characterRoot = model.transform;
            _eyes.queueProcessor = model.GetComponent<QueueProcessor>();
            if(_eyes.queueProcessor == null)
                _eyes.queueProcessor = model.AddComponent<QueueProcessor>();
            ...
            _eyes.heads[0].expData.components[0].easing = LerpEasings.EasingType.Linear;
            ...
            _eyes.headClamp = new Vector3(90.0f, 120.0f, 0.0f); 
            ...
            var smrParams = GetSMRandBlendIndex("Eye_Blink");
        ...
        private (SkinnedMeshRenderer smr, int blendIndex) GetSMRandBlendIndex(string blendShapeName)
        {
