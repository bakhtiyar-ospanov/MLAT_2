...
namespace Modules.WDCore
{
    public static class UIExtensions {
        ...
        public static void ScrollToTop(this ScrollRect scrollRect, RectTransform target, RectTransform.Axis axis = RectTransform.Axis.Vertical)
        {
            // The scroll rect's view's space is used to calculate scroll position
            var view = scrollRect.viewport ?? scrollRect.GetComponent<RectTransform>();
            ...
            if (axis == RectTransform.Axis.Vertical) {
                ...
                var offset = viewRect.max.x - elementBounds.max.x;
                var scrollPos = scrollRect.horizontalNormalizedPosition - scrollRect.NormalizeScrollDistance(0, offset);
                scrollRect.horizontalNormalizedPosition = Mathf.Clamp(scrollPos, 0, 1);
            }
        }
        
        public static (Bounds, float) FitToBounds(Camera cam, Transform t)
        {
            var targetBounds = EncapsulateBounds(t);
        
            var boundRadius = targetBounds.extents.magnitude;
            var cameraView = (2.0f * Mathf.Tan(0.5f * cam.fieldOfView * Mathf.Deg2Rad));
            var finalDistance = 3.0f * boundRadius / cameraView;
            return (targetBounds, finalDistance);
        }
        
        public static Bounds EncapsulateBounds(Component transform)
        {
            Bounds bounds;
