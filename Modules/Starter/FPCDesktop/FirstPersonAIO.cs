...
public class FirstPersonAIO : MonoBehaviour {
    ...
    private void Update(){
        if(Cursor.lockState != CursorLockMode.Locked && !Input.touchSupported)
             return;
        ...
        if(enableCameraMovement && !controllerPauseState){
            ...
            if (Input.touchSupported && fixedTouchField != null)
            {
                mouseYInput = fixedTouchField.touchDist.y;
                mouseXInput = fixedTouchField.touchDist.x;
            }
            else if (cameraInputMethod == CameraInputMethod.Traditional || cameraInputMethod == CameraInputMethod.TraditionalWithConstraints){
    ...
    private void FixedUpdate(){
        ...
        if(Cursor.lockState != CursorLockMode.Locked && fps_Rigidbody.velocity == Vector3.zero && !Input.touchSupported)
            return;
        ...
        MoveDirection = (transform.forward * inputXY.y * speed + transform.right * inputXY.x * walkSpeedInternal);
            ...
            if (Input.touchSupported && leftJoystick != null)
            {
    ...
    public class FPAIO_Editor : Editor{
        ...
        #region Movement Setup
            ...
            EditorGUILayout.Space();
            EditorGUILayout.Space();
            t.leftJoystick = (FixedJoystick)EditorGUILayout.ObjectField(new GUIContent("Left joystick","for move"),t.leftJoystick,typeof(FixedJoystick),true);
            t.fixedTouchField = (FixedTouchField)EditorGUILayout.ObjectField(new GUIContent("fixedTouchField","for look"),t.fixedTouchField,typeof(FixedTouchField),true);
